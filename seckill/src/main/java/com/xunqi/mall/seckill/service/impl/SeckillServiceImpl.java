    ...
    public String kill(String killId, String key, Integer num) throws InterruptedException {
        ...
        if (currentTime >= startTime && currentTime <= endTime) {
            ...
            if (randomCode.equals(key) && killId.equals(skuId)) {
                ...
                //2. 判断重复下单
                //把订单信息先放入redis预存，根据用户id和商品id存入。如果重复了，那就是重复下单
                SeckillOrderTo order = (SeckillOrderTo) redisTemplate.opsForValue().get("order:" + user.getId() + ":" + redisTo.getSkuId());
                if (order != null) {
                    return error(REPEATE_ERROR);
                }else{
                    redisTemplate.opsForValue().getAndSet("order:" + user.getId() + ":" + redisTo.getSkuId(),1);
                }

                //3. 设置秒杀大闸
                redistemplate.opsForValue().set("door_count"+productId, stock.getStock()*5);
                // 发放令牌时，先去redis获取当前大闸剩余令牌数
                int bigDoor = redistemplate.opsForValue().get("door_count"+productId);
                if (bigDoor <= 0) {
                    // 抛出一个异常
                    throw new exception;
                }else {
                    //得到大闸通过的令牌
                    String tocken = UUIDUtils.getUUID()+productId;
                    // 用户只有拥有这个token才有资格下单
                    redistemplate.opsForValue().set(userToken, token);
                    //大闸数量减1
                    redistemplate.opsForValue().get("door_count"+productId).decrement();
                }
                ...
                if (count > 0 && num <= seckillLimit && count > num ) {
                    ...
                    if (aBoolean) {
                        ...
                        //保证Redis中还有商品库存
                        if (semaphoreCount) {
                            //5. rabbitMQ实现消息队列进行异步创建订单，修改数据库
                            //创建订单号和订单信息发送给MQ
